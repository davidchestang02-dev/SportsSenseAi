...
export function buildMlbPropsGames({ bundle, pregameSlate, weatherBoard, includeExampleFallback = false }) {
  ...
  return sourceGames.map((game, index) => {
    ...
    const contractGame = {
      ...
      startTime: game.startTime || game.date || null,
      awayTeam: {
        id: Number(game.teams?.away?.id || game.awayTeam?.id || 0) || 1000 + index,
        abbr: game.teams?.away?.abbreviation || game.awayTeam?.abbr || "AWY",
        name: game.teams?.away?.name || game.awayTeam?.name || "Away Team"
      },
      homeTeam: {
        id: Number(game.teams?.home?.id || game.homeTeam?.id || 0) || 2000 + index,
        abbr: game.teams?.home?.abbreviation || game.homeTeam?.abbr || "HME",
        name: game.teams?.home?.name || game.homeTeam?.name || "Home Team"
      },
      probablePitchers: {
        away: {
          id: Number(game.probablePitchers?.away?.id || 0) || 3000 + index,
          name: game.probablePitchers?.away?.name || game.probablePitchers?.away?.fullName || `${game.teams?.away?.abbreviation || "Away"} Starter`
        },
        home: {
          id: Number(game.probablePitchers?.home?.id || 0) || 4000 + index,
          name: game.probablePitchers?.home?.name || game.probablePitchers?.home?.fullName || `${game.teams?.home?.abbreviation || "Home"} Starter`
        }
      },
      weather: {
        tempF: Number(weather?.hourly?.[0]?.tempF || 70),
        windSpeed: Number(weather?.hourly?.[0]?.windSpeed || 8),
        windDir: Number(weather?.hourly?.[0]?.windDirDeg || 225),
        isDome: Boolean(weather?.isDome || game?.neutralSite === false ? false : false)
      }
    };
